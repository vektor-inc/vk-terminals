...
- [ 不具合修正 ] タスク保存後、VK Orchestrator 側の反映に時間がかかると誤ってエラーになり、再保存で同じ変更が二重送信される不具合を修正。反映待ちの間は保存・編集を無効にしたまま待機し、数分（既定5分）経っても反映されない場合だけエラーにするよう変更（[#406](https://github.com/vektor-inc/vk-terminals/issues/406)）
