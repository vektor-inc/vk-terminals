...
//
// PR #365 の仕様（司・植草合意）: 罫線テーブルのセルの右端（縦線）に接している
// URL はリンク化されない（切り詰められているかどうかに関わらず）。
// なお「(URL) がセルの右端に来る」ケース（例:
// "│ #399 (https://.../pull/399) │ x │"）は司・植草で扱いを検討中のため、
// このファイルでは対象にしない（結論が変わり得るケースをテストで固定しない）。
