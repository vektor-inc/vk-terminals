...
- [ 不具合修正 ] 設定パネルの ⚙ ボタン・スマホ用画面のタスク一覧の開閉ボタンなど、キーボードで選んだときにフォーカス位置を示す枠が表示されない対話部品があった不具合を修正。今後追加する部品にも自動で枠が付くよう仕組みを変更（[#302](https://github.com/vektor-inc/vk-terminals/issues/302)）
