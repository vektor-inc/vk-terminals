...
- [ 機能追加 ] Claude の利用状況を公式 usage API から取得し、設定モーダルの「使用状況」タブに現在のセッション%・週間制限%（すべてのモデル）とリセット時刻をプログレスバーで表示。取得不可時（未ログイン・オフライン等）は従来のトランスクリプト集計（現在の5時間ブロック・自己ピーク比）表示へ自動フォールバック（[#73](https://github.com/vektor-inc/vk-terminals/issues/73)）
- [ 機能追加 ] 公式の使用率（セッション・週間のいずれか）が 80% を超えたとき、タイトルバーの歯車ボタンに警告ドットバッジ（80〜90% アンバー / 90%〜 赤）を表示（[#73](https://github.com/vektor-inc/vk-terminals/issues/73)）
- [ 仕様変更 ] タイトルバー左のトークン使用量インジケータを廃止し、設定モーダルを「使用状況｜設定」のタブ構成に変更（既定タブは使用状況・歯車ボタンは常時表示）。モバイルページの使用量カードは公式データ取得時にセッション%・週間%の2バー表示に（[#73](https://github.com/vektor-inc/vk-terminals/issues/73)）
