...
- [ 不具合修正 ] Claude Code の AskUserQuestion（「❯ 1. … / 2. … / Enter to select」形式の数字選択肢 UI）で「🟡 入力待ち」が点灯しない不具合を修正。`WAITING_PATTERNS` に AskUserQuestion フッター文言（`Enter to select` / `↑/↓ to navigate` / `Esc to cancel`）、数字選択肢（`❯ 1. ラベル`）、行末が全角「？」で終わる質問文の検知パターンを追加（[#46](https://github.com/vektor-inc/vk-terminals/issues/46)）
