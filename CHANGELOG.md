...
- [ 不具合修正 ] 罫線テーブルのセル内で URL がセル幅を超えて折り返されると、途中で切り詰められた断片だけがリンク化され、クリックすると 404 になる不具合を修正。あわせて、セルの右端（縦線）に接する URL は、後ろに他の文字が続かない限りリンク化の対象外へ変更（[#361](https://github.com/vektor-inc/vk-terminals/issues/361)）
