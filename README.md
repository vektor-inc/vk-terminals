...
- 移動先が**行き止まりのタブ**（開いても「このタブに表示できる設定項目はありません。」という案内だけが表示されるタブ）の場合、その `tabLink` は表示されません（押した先に何も無いボタンは、同じ画面の他の移動ボタンまで信用できなくします）。取り除いたときは開発者コンソールに `[settings]` から始まる警告を出すので、書いたはずのボタンが出ないときはそちらで原因を確認してください。
- `tabLink` を取り除いた結果、それを書いていたタブ自身が行き止まりのタブになった場合は、そこを指す `tabLink` も同じように取り除きます（連鎖が収束するまで繰り返します）。取り除かれるのは移動ボタンだけで、タブ自体はタブバーに残るため、自分でそのタブを開くことはできます。
...
- タブに `note` を書いた場合は、そのタブを行き止まりのタブとして扱いません（`note` に代替手段を書いた直後にそれを打ち消す案内を出さないため）。案内は表示せず、`tabLink` の移動先としても有効です。
