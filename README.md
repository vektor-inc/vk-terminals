...
HTTP API はすべてのリクエストで、接続先を示す Host ヘッダが `localhost`、ループバックアドレス、設定した `apiHost`、または実際の待ち受けアドレスのいずれかであることも確認します。一致しないアクセスは `403 {"error":"forbidden host"}` で拒否します。`apiHost` が `0.0.0.0` または `::` の場合は到達に使うアドレスを列挙できないため、この確認を省略し、必須となるアクセストークン認証で保護します。
...
- `content` の先頭（親となる `<h3>` がまだ出ていない位置）に `level: 4` を書いた場合は、`3` に繰り上げて表示します。モーダル全体の見出し（`<h2>`）の直下が `<h4>` になると見出しレベルが 1 段飛び、スクリーンリーダーで階層をたどれなくなるためです。`text` を書き忘れた見出しは表示されないため、親としても数えません（その直後の `level: 4` は `3` になります）。
...
- ボタンを表示させたい場合は、移動先のタブに設定項目（`groups` のフィールド）・説明ブロック（`content`）・`note` のいずれかを用意するか、`tab` の指定を中身のあるタブへ変えてください。そもそも不要な導線であれば `tabLink` ブロックごと削除します。
