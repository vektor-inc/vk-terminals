...
- [ 仕様変更 ] ペイン右上の PR ボタンが「オープン（緑）／マージ済み（紫）」の2状態しかなく、VK Orchestrator からのマージ待ち通知を反映できなかったため、「PR が出ただけ（灰）／マージ待ち（青）／マージ済み（紫）」の3状態に変更。これまで緑だった「PR が出ただけ」の表示が灰色に変わります（[#363](https://github.com/vektor-inc/vk-terminals/issues/363)）
- [ デザイン不具合修正 ] 格納（サイドバー格納）カード内のマージ済み PR ボタンが、格納カードの背景色に対して基底カラー適用のままコントラスト比 4.22:1 と AA（4.5:1）未達だったため、文字・境界色を明るいトークンへ個別上書きして AA を満たすよう修正
- [ デザイン不具合修正 ] モバイル表示の PR ボタンの枠線が半透明のままで、状態によって WCAG 2.1 AA の UI コンポーネント基準（3:1）を満たしたり満たさなかったりする不安定な状態だったため、文字色と同じ不透明トークンへ変更（マージ済み表示の既存不具合もあわせて修正）
