...
1. パソコンのタイトルバー右端の ⚙ →「モバイルから確認」タブを開き、「アクセストークン」欄の下にある「初回登録用の URL を表示」を押します。表示された URL（トークン付き）をコピーします。
...
トークンは初回起動時に自動生成され `~/.vk-terminals/config.json` の `apiToken` に保存されます。値はタイトルバー右端の ⚙ →「モバイルから確認」タブの「アクセストークン」欄（既定は伏せ字。「表示」で表示、「コピー」でコピー）から確認できます。認証が必要な状態でトークンを付けずにリクエストすると `401 {"error": "unauthorized"}` が返ります。
