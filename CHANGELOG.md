...
- [ 不具合修正 ] 罫線テーブルのセル内で URL がセル幅を超えて折り返されると、途中で切り詰められた断片だけがリンク化され、クリックすると 404 になる不具合を修正。あわせて、セル右端の URL の直後に句読点（`.` `,` 等）だけを挟んで縦線が続く場合は、切り詰めの有無に関わらずリンク化の対象外に変更（`(URL)` のように閉じ括弧で終わる場合は従来どおりリンク化）（[#361](https://github.com/vektor-inc/vk-terminals/issues/361)）
