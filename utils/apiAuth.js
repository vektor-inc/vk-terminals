...
// 対して送ってしまう。
//
// 【この導出値化が守るもの・守らないもの（PR #315 再レビュー指摘・修正-2）】
// 導出値は isAuthorizedRequest が Authorization: Bearer 経由でもそのまま受理する
// （下記参照）ため、本体トークンと同じ権限を持つ完全な資格情報である。ポート違いの
// 別サービスへ Cookie が漏れた場合、その受け取り側のログに残る値で新規ペイン作成・
// コマンド送信など何でもできてしまう点は、トークン本体を直接 Cookie に載せていた
// 場合と変わらない。
// この導出値化が守るのは、その漏えい経路で「マスター値」（設定パネルに表示され、
// AirDrop 等でスマホへ運ぶ本体トークン。これが漏れると再発行しない限り無効化
// できない）まで一緒に読まれることだけである。HMAC の鍵に本体トークンを使うことで、
// 導出値からトークン本体を逆算できないようにしている。'cookie-v1' は用途を固定する
// ためのドメイン分離用文字列で、将来 Cookie 以外の派生値が必要になった場合に
// 別の文字列を使えば値が衝突しない。
//
// この構造（暗号化されない HTTP ＋ ホスト単位で送られる Cookie）では、上記の
// ポート違い漏えいそのものは構造的に防げない（`__Host-` プレフィックスは HTTPS が
// 必須で使えず、`tailscale serve` 経由かどうかもアプリ側から判別できない）。
// 運用上の回避策は README の「セキュリティ上の注意」に記載している。
// 導出値を「秘密ではない値」として扱わないこと（ログ・画面・URL に出さない）。
