...
- [ 不具合修正 ] 罫線テーブル以外の通常のペイン出力で、URL がペイン幅ちょうどの位置で実改行により2行に割れると、1行目の断片だけがリンク化されクリックすると 404 になる不具合を修正。1行目がペイン幅いっぱいに埋まっている場合は次行の折り返し判定に関わらず連結するが、連結後の URL が境界より前でホスト名を書き終えていない場合（ホストのすり替わりを防ぐため）は断片も含め一切リンク化しないよう変更（[#368](https://github.com/vektor-inc/vk-terminals/issues/368)）
