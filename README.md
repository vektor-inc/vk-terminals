...
ペインが入力待ち・マージ待ちになった瞬間に、登録済みのスマートフォンへ Web Push（ブラウザが標準で持つ、ページを閉じていても通知を受け取れる仕組み）で通知を送れます。**方法 2（`tailscale serve` で公開する）で開いている場合のみ**使えます。方法 1（`apiHost` に Tailscale IP を指定する方法）は HTTPS ではないため、ブラウザの仕様により通知を使えません。

1. 方法 2 の手順で `tailscale serve` を実行し、表示された `https://` のアドレスをスマートフォンで開きます。
2. iPhone の場合は、Safari の共有ボタンから「ホーム画面に追加」を行い、追加したアイコンから開き直してください（iOS 16.4 以降の仕様により、ホーム画面に追加したページでないと通知を受け取れません）。Android の Chrome ではこの操作は不要です。
3. モバイルページ最上部の「🔔 通知」カードで「通知を受け取る」を押し、ブラウザの確認ダイアログで「許可」を選びます。

補足:

- Safari で登録済みの状態からホーム画面に追加すると、Cookie が引き継がれず端末登録をやり直す必要がある場合があります。その場合は追加後の画面の案内に従って再登録してください。
- `tailscale serve` の公開アドレス（マシン名や tailnet 名）が変わると、登録済みの通知は届かなくなります。その場合はモバイルページで「通知を受け取る」を押し直してください。
- 入力待ち・マージ待ちそれぞれの通知は、設定パネルの「設定」タブ「基本」グループの `notifyOnWaiting` / `notifyOnWaitingMerge`（既定は両方 ON）でオン・オフできます。
- VK Terminals を動かしている Mac がインターネットにつながっていないと、通知は送れません（送信先はプッシュ配信サーバーのため、tailnet 内の通信だけでは届きません）。
...
- 列が `.settings-tab-panel` の実効幅（実測 522px）に収まらない場合は、表全体が横スクロールします。スクロール領域は Tab キーで到達でき、フォーカスした状態で ArrowLeft / ArrowRight（少しずつ）・Home / End（両端へ）でも操作できます。右端には「まだ続きがある」ことを示す固定のフェードを常時表示します（マウスホイール・トラックパッドでもスクロールできます）。
...
#### `GET /api/push-public-key`（issue #396）

モバイルページが `PushManager.subscribe()` に渡す VAPID 公開鍵を返します。モバイルページの内部実装用で、外部プログラムから呼ぶ想定のものではありません。

```bash
curl -s http://127.0.0.1:13847/api/push-public-key -H 'Authorization: Bearer <アクセストークン>'
```

#### `POST /api/push-subscribe`（issue #396）

モバイルページの「通知を受け取る」ボタンが、`PushManager.subscribe()` で得た購読情報（この端末への通知の宛先）を登録するために呼びます。同じ `endpoint` が既にあれば置き換えます（重複登録しません）。

```bash
curl -s -X POST http://127.0.0.1:13847/api/push-subscribe \
  -H 'Authorization: Bearer <アクセストークン>' \
  -H 'Content-Type: application/json' \
  -d '{"subscription": {"endpoint": "https://fcm.googleapis.com/fcm/send/...", "keys": {"p256dh": "...", "auth": "..."}}}'
```

#### `POST /api/push-unsubscribe`（issue #396）

モバイルページの「停止」ボタンが、指定した `endpoint` の購読情報を削除するために呼びます。存在しない `endpoint` を指定してもエラーにはなりません（冪等）。

```bash
curl -s -X POST http://127.0.0.1:13847/api/push-unsubscribe \
  -H 'Authorization: Bearer <アクセストークン>' \
  -H 'Content-Type: application/json' \
