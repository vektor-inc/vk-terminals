...
- `cells` の各要素（セル）に置けるのは次の 4 種類だけです。リンク・ボタン・任意の HTML は置けません。
  ...
  - `{ "type": "fieldValue", "key": "<フィールドの key>", "map"?: [{ "value": "…", "label": "…", "tone"?: "…" }] }`：**今の入力値**（保存前の値を含む）を表示します。入力のたびに再計算され、画面上の表示も自動で更新されます。`map` の `value`（文字列化して比較）に現在値が一致すればバッジとして表示し、一致しなければ現在値をそのままテキスト表示します（未知の値を隠さないための既定動作です）。値が空・未設定の場合は「未設定」と表示します。参照する `key` は表と同じタブに限らず、モーダル内の任意のタブのフィールドを指せます。
  - `{ "type": "savedValue", "key": "<フィールドの key>" }`：**設定ファイルに実際に保存されている値**を表示します（今の入力値ではありません）。renderer だけでは分からないため、モーダルを開いたとき（および再試行時）に IPC で都度取得します。取得中はそのセルだけ「確認中…」、失敗した場合はそのセルだけ「取得できません」＋再試行ボタンになります（表全体は止まりません）。**セキュリティ上の制約**: `key` はそのディスクリプタ自身が `groups[].fields[].key` として宣言しているキーだけが読めます。宣言されていないキーや `__proto__` 等の危険なキーセグメントを含むキーはエラーになります（任意のキー・任意のパスを読み出せる窓口ではありません）。
...
- `danger`（任意・既定 `false`）：`true` にすると、危険な一括操作であることを示す強調色（`.settings-content-apitoken-btn-danger` と同じ危険色トークン）でボタンを表示します。
- 押したときの挙動:
  - 対象の欄のうち、**現在無効化中（`disabledWhen` 等でグレーアウト）の欄・存在しないキーの欄は対象から自動的に除外**されます（無効化中の値を固定する既存の仕組みと競合しないための必須の挙動です）。すべての対象が無効化されている場合、ボタンは操作できない状態（`aria-disabled="true"`）になり、「対象の項目がすべて無効化されているため使用できません」という理由が添えられます。
  ...
  - 完了すると、画面下部のトーストで結果を要約します（例:「3件の設定を切り替えました。保存するには「保存」を押してください。」／一部が対象外だった場合は「2件の設定を切り替えました（1件は現在変更できない状態のため対象外です）。保存するには「保存」を押してください。」）。
