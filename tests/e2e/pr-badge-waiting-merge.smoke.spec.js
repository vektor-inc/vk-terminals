...
// issue #377: 2 テストとも env/config の指定なしで launchApp を呼んでいるため、起動を
// 1 回に共有する（stash-header.smoke.spec.js と同じ考え方）。/api/set-title は毎回
// 未指定フィールドを false 扱いにする（main.js の parseStrictBoolFlag。前回値を引き継がない）
// ため、1 つ目のテストが termId "1" に残す prMerged/waitingMerge の値は 2 つ目の
// postSetTitle 呼び出しで即座に上書きされ、2 つ目のアサーションには影響しない。
// ただし 1 つ目はグリッド上のペイン、2 つ目は格納カードでの表示を見るため、
// 2 つ目の末尾で termId "1" をグリッドへ戻し、#348 の不変条件（各テスト終了時点で
// 最初のペインだけがグリッドに残る）を保つ。
test.describe.serial('PR バッジのマージ待ち表示（issue #363）', () => {
  let app;
  let win;
  let tmpRoot;
  let port;
  ...
  test.beforeAll(async () => {
    ...
    // これらのテストは HTTP API と renderer 反映の統合パスだけを見るため、Claude CLI の
    // 有無に依存させない素のシェル（--no-claude）で起動する。いずれもヘルパーが行う。
